...
function Category() {
    ...
    const handleAddCategory = async() =>{
      if(!categoryName){
        ...
        let body = {
          categoryName:categoryName,     // if the key name & the obj name is same, you can kjust type in 'categoryName,' instead.
          allVideos:[]
        }
  ...
  const videoDrop = async (e,id)=>{
    console.log("===on drop===");
    // get video Id that we passed from videocard
    const videoid = e.dataTransfer.getData("videoID");
    console.log(`Video with id ${videoid} need to be put inside category with id ${id}`)
  ...
  const dragOver = (e)=>{
      // onDragOver method will trigger page refresh. So in order to prevent that, we pass the videoID may lost
      e.preventDefault();
      console.log("===Inside drag over===")
  }
  return (
    <>
     <button className='btn btn-warning' onClick={handleShow}>Add New Ctegory</button> 
     {
              ...
              >
                   ...
                   </div>
                   {
                    ...
                    item.allVideos?.map(card=>(
                      <Videocardnew displayVideo = {card}/>
